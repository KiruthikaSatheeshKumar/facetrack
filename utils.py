...
def recognize(image, TOLERANCE, return_location=False): 
    ...
    for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
        ...
        if True in matches:
            ...
            user_id = database[match_index]['id']
            distance = round(distances[match_index], 2)
            matched_location = (top, right, bottom, left)
            ...
            cv2.putText(image, 'Unknown', (left, top-10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0,0,255), 2)

    if return_location:
        return image, name, user_id, matched_location
    return image, name, user_id
